...
lattice. Using these methods we show that with one possible
...
label it $L_1$.  A search of \gaps Small Groups Library~\cite{GAP4}
reveals that the smallest group $G$ in whose subgroup lattice
$N_5$ appears as an upper interval is \code{SmallGroup(216,153)}. It is easy
...
permutation. For such a lattice $L$, a minimal representation comes from a
group $G$ acting on a set $X$, which is a group representation
of the type described in~(\ref{eq:1}).
Specifically, $L \cong \lb G_x, G\rb$, where $G_x$ denotes the
stabilizer of an arbitrary element $x\in X$.
Finding a representation is typically harder in such cases,
...
refer to a lattice by the name of its universe.
...
lattices of finite algebras. The notation 
$\sL_{grp}$ will stand for the class of finite lattices that
are isomorphic to intervals in subgroup lattices of finite groups.
...
%% In what sense is a representation of a finite lattice \emph{minimal}?
The \defn{size} of a representation $\Con(\bA) \cong L$ is the cardinality
$|A|$ of the universe of $\bA$.  We call such a representation
\defn{minimal} if $\Con(\bB) \cong L$ implies $|A| \leq |B|$.
%% Thus, $\Con(\bA) \cong \bL$ is a minimal representation of $\bL$ if and only if we can prove
%% that $\Con(\bB) \cong \bL$ implies $|A| \leq |B|$.
...
index $n = [G:H]$ and satisfying $L \cong \lb H, G \rb$?
...
The following is a list of operations under which $\sL_{con}$ is known to be closed,
...
The class $\sL_{con}$ of lattices isomorphic to congruence lattices of finite
...
isomorphic to $\EqX$. The lattice isomorphism is the function
...
$y(2) = y(4)$. That is,
%% \begin{align*}
%% Xf &= \{\bx f \mid \bx\in S^5\} = \{(x(0), x(0), x(2), x(3), x(2)) \mid \bx\in S^5\}\\
%% &=\{\bx\in S^5 \mid \ker f \leq \ker \bx\}.
%% \end{align*}
...
\begin{lemma}
  ...
  The map $f \mapsto Xf$ is a dual lattice isomorphism from $\Eq(n)$ onto the
...
\begin{proof}
  ...
  of the lattice $Eq(n)$.  That is, 
  $\Con \<S^n/D, S^n\> \cong (Eq(n))'$.
...
\begin{remarks}\
  ...
  \item 
    ...
    %% there corresponds a subgroup $Xf \in \lb D, S^n\rb$ such that 
    %% $\bx\mathrel{\theta} \by$ iff $\bx^{-1}\by = \bz\in Xf$.
    %% Now if $\hat{\phi}\in \hat{F}$, then $(\hat{\phi}\bx, \hat{\phi}\by) \in \theta$ iff 
    %% \[ (x({\phi(0)})^{-1}y(\phi(0)), \dots,  x(\phi(n-1))^{-1} y(\phi(n-1))) =
    %% (z(\phi(0)),z(\phi(1)),\dots, z(\phi(n-1)))\in Xf. \]
    %% Better:
...
using primitive positive formulas.  We provide an alternative proof for two reasons.
First, the lemma is a useful tool for proving that certain finite lattices 
are representable as congruence lattices, and providing a proof helps to keep the
paper self-contained. More importantly, however, our proof is more direct than
the proof via primitive positive fomulas in that we construct the algebras
that the lemma claims exist.  For our purposes this is preferable
since, when trying to prove that a given lattice is representable, whenever possible we 
...
Is $L$ representable (isomorphic to $\Con(\bA)$ for some finite algebra $\bA$)?
...
the lattices $L_6$ and $L_7$ diagrammed in Figure~\ref{fig:L6and7}
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
\caption{$L_{6}$ and $L_7$}\label{fig:L6and7}
\end{figure}
...
As mentioned above \Palfy and \Pudlak gave conditions on a lattice $L$ 
such that if $L \cong \Con \<A,F\>$ then the nonconstant elements of $F$ are 
...
In \cite{Shareshian2003} John Shareshian gave a class of finite lattices that
...
An example of such a lattice appears in the diagram in Figure~\ref{fig:shareshian}.
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
\label{fig:shareshian}
\caption{A possibly non-representable lattice.}
\end{figure}
...
$11$ \marginpar{PJ: should this be less than or equal to 11???} are known to be minimal-size algebras that produce the corresponding
