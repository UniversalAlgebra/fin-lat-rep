...
\begin{remarks}
  ...
  Then $[\alpha, 1_A] \cong \Con \bA/\alpha$. By~(1), the dual of $[\alpha, 1_A]$ is
  representable. %say, $\ell' = \Con \bB$. 
  Now take the filter above $\beta'$ in the dual of  $[\alpha, 1_A]$
  (where $\beta'$ is the
...
\begin{center}
  \begin{tikzpicture}[scale=.4]
    ...
    \draw (alpha) node [left]{$\alpha$};
    \draw (beta) node [right]{$\beta$};
    %% Put it inside a potato (since its a sublattice)
    %\draw[dotted] (bottom) to [out=12.5, in=-12.5] (top) to [out=192.5, in=162.5] (bottom);
...
\begin{enumerate}
\item[(B$'$)]
If $\varphi: L \to L$ is meet-preserving and $\varphi(x) > x$ for
...
Using techniques similar to those above, one can show that,
except for $\bL_2 = \bM_3$, $\bL_5$, $\bL_{15}$, 
$\bL_{19}$, and $\bL_{34}$,  none of the lattices 
$\bL_i$, $i = 1,\ldots, 35$, diagrammed  below can be 
represented with an intransitive permutation group. 
We do not know if $\bL_{19}$ has an intransitive representation but each of the 
other exceptions do. 
The four element algebra given for $\bL_5$ in the diagram 
is minimal but not a group action. Nevertheless,
for $A$ of size six the operations $(1,2,0,4,5,3)$ and $(0,1,2,3,5,4)$ do give
a represntation by an intransitive group.
\marginpar{Figure out if $\bL_{19}$ has an intransitive rep!} 
