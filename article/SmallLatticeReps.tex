...
see~\cite{alvi:1987}.
The \Palfy-\Pudlak theorem, however, does not prove that, if 
...
group (although the opposite implication is obviously true) so
...
In this paper we review some of the known methods, as well as 
some new methods, of constructing a finite
algebra whose congruence lattice is $L$, for a given finite
lattice $L$. Using this methods we show that with the possible
exception of one ($L_{10}$ in the last section), every 
...
\todo{define group representation of a lattice. Is transitivity
required?}

Not surprisingly many lattices with a group representation
can be represented as a congruence lattice of a much smaller
algebra. For example, the hexagon lattice, $L_6$ in the 
last section, is the congruence lattice of an algebra 
with 6 elements.
Aschbacher~\cite{Aschbacher2008} has found groups
$H < G$ such that the interval from $H$ to $G$ is $L_6$, but 
$[G : H]$, the size of the algebra, is considerably larger.

\todo{can we say that Group(216,153) is the smallest group
with $N_5$ as an interval?} 


But for other lattices it can be shown that in a minimal 
representation every nonconstant operation must be a 
permutation. Thus, for these lattices, a minimal representation
is a group acting on a set. Naturally for these lattices 
finding a representation is harder, and finding a minimal
representation can be especially hard.
...
Let $\bA,\bB$ be two finite algebras of cardinality $m$ and $n$ respectively,
and let $L=\bCon(\bA)$ and $M=\bCon(\bB)$. 
In \cite{Snow:2000} it is proved that $L\oplus_a M$ is isomorphic to the
congruence lattice of some finite algebra $\mathbf C$. Although the algebra $\mathbf C$ is
not explicitly constructed, it is based on the direct product of $\bA$ and $\bB$,
hence the ordinal sum is a sublattice of $\bEq(mn)$. Here we give a different
construction of $\mathbf C$ that leads to a tighter representation of $\bCon(\mathbf C)$ as
a sublattice of $\bEq(m+n-1)$.  
%If $\bA$ and $\bB$ are minimal-size algebras that have congruence lattices
%isomorphic to $\bL$ and $\bM$ respectively, then $\mathbf C$ will also be a
%minimal-size algebra with congruence lattice isomorphic to $\bL\oplus_a\bM$. 
...
\begin{proof}
  ...
  isomorphic to $\bCon(\bA)$ and $\{\theta\in\Con(\mathbf C):\theta\le\alpha\}$ 
...
Now assume $\bA$ and $\bB$ are minimal-size algebras with congruence lattices
isomorphic to $L$ and $M$ respectively. Then it seems likely that the
algebra $\mathbf C$ above is minimal in size with respect to having a congruence
lattice isomorphic to $L\oplus_a M$. Suppose $\mathbf C'$ is a smaller algebra
...
isomorphic to $M$, hence $|\mathbf C'/\alpha|=|\bB|=$ number of blocks in
...
\defn{representable lattice}.  On the other hand, given a sublattice $L_0\leq \EqX$, 
...
is a \defn{minimal representation of} $\bL$ if the size
...
\node (2) at (-0.75,1) [draw, circle, inner sep=1.5pt] {};
\node (3) at (-0,2) [draw, circle, inner sep=1.5pt] {};
\draw[font=\scriptsize] (0,-.5) node {$\lb 1, C_{10}\rb$};
...
\begin{tabular}{l}
Finite algebra with\\
108 elements known\end{tabular}
\end{tabular}
...
&
\begin{tabular}{l}
$\bB_{20}=$ SmallGroup(216,153) in GAP\\
$|\bB_{20}|=216$
\end{tabular}
\end{tabular}
