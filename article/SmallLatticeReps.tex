...
is a congruence lattice of a finite algebra that it 
...
with 6 elements. Palfy~\cite{Palfy1988} and
...
%We call it the \defn{closure method}, and it is feasible for small lattices
...
\defn{representable lattice}.  On the other hand, given a sublattice 
...
\<X, \lambda(L_1)\>$.\marginpar{$\lambda(L_1)$ is only defined on the next page?}
...
There is, however, a separate notion of smallness of a representation that is
worth considering.  Suppose we know that a lattice $L$ is representable as
an upper interval in the subgroup lattice of a finite.  We could ask, ``what is
the smallest group $G$ such that $L \cong \lb H, G \rb$ for some $H\leq G$?''
Alternatively, we could ask, ``what is the smallest number $n$ such that there are
finite groups $H\leq G$ with index $n = [G:H]$ and satisfying
$\bL \cong \lb H, G \rb$?''  The reason this question seems more 
natural is because the resulting algebra representing $L$ is the group
$G$ acting on the set of cosets of $H$, and this algebra has $n = [G:H]$
elements.  In this context, the index $[G:H]$ seems more relevant than the size
of the group $G$.

This begs the question, if we discover that $G$ is a group of minimal order
among those in whose subgroup lattices $L$ appears as an upper interval,
could it be that there is another, perhaps larger group in whose subgroup
lattice $L$ appears, but with a smaller index?  The answer to this question is
yes, as we now show by providing an example of this phenomenon. 
