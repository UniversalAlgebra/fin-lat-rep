...
Let $L$ be a finite lattice. We consider three representation
...
It is shown in 
\textcolor{purple}{refsXXX, and also later in the paper,}
that (3) and(4) are equivalent; that is, a lattices satisfies~(3)
if and only if it satisfies~(4). Obviously, (3) implies (2) which 
implies~(1).

By the celebrated theorem of \Palfy and P.~\Pudlak \cite{PalfyPudlak1980}, (1)
holds for \emph{all} lattices if and only if (4) holds
for \emph{all} lattices. Nevertheless, we do not know, for example,
if (1) and (4) are equivalent.
...
(3) and (4), the algebra constructed from $H \le G$ (subgroup) has
as its elements, the left cosets of $H$ (its operations are
the elements of $G$ acting by left multiplication). So in the case of~(4),
...
permutation, that is, the representation is of type~(2),
or~(3) if the nonconstant operations generate a transitive permutation group.
...
a lattice can be represented as in (2) where the 
...
lattices of finite algebras, that is, all lattices that satisfy property~(1). 
The notation 
...
As defined above, the  \defn{size} of a representation $\Con(\bA) \cong L$ 
is the cardinality
...
is defined as $[G : H]=|G|/|H|$, which, after all, is the size of the 
corresponding algebra in the equivalence of properties (3) and~(4). 
...
\begin{lemma}[Lem.~1.6B~\cite{Dixon:1996}]
